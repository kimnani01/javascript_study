...
let a = 123;
let b = 0;
console.log(b && a); //0 모두 참이어야 참 반환(거짓)
...
number_btn.addEventListener('click',()=>{
    if(user_number.value%2==0){alert(`당신이 입력한 ${user_number.value}는 짝수입니다.`)
    }else{
        alert(`당신이 입력한 ${user_number.value}는 홀수 입니다.`)
    }
})
console.log('---------------------')
...
walk_btn.addEventListener('click', ()=>{
    if(walk.value>=5000){alert(`목표 달성! ${walk.value}보 걸으셨네요!`)
    }else{
        alert(`목표 달성까지 ${5000-walk.value}보가 부족하네요!`)
    }
})
//----------------------------
